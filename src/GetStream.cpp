   /*
* Copyright(C) 2010,Hikvision Digital Technology Co., Ltd 
* 
* File   name：GetStream.cpp
* Discription：
* Version    ：1.0
* Author     ：panyd
* Create Date：2010_3_25
* Modification History：
*/

#ifdef _WIN32
#include <windows.h>
#elif defined(__linux__) || defined(__APPLE__)
#include   <unistd.h> 
#endif

#include "HCNetSDK.h"
#include "public.h"
#include <stdio.h>
#include <time.h>
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <unistd.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>  
#include <netdb.h>  
#include <fcntl.h>  
#include <sys/epoll.h>
#include <pthread.h>
#include "thread.h"
#include "EasyPusherAPI.h"
#include "EasyTypes.h"




#define UDP_TEST_PORT       50001
#define TDP_TEST_PORT       50002
#define UDP_SERVER_IP       "127.0.0.1"

FILE *g_pFile = NULL;
int sock_fd = NULL;
int accept_fd = NULL;
int timestamp = 0;

void creat_udpsock()
{
	int sockfd;   
	if ((sockfd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		perror("socket");
		exit(1);
	}
	sock_fd = sockfd;
	return;
}

int writefile(BYTE *pPacketBuffer,DWORD nPacketSize,DWORD dwDataType)
{
	printf("dwDataType : %d \n",dwDataType);
	
	if (dwDataType  == NET_DVR_SYSHEAD)		
		{	
			g_pFile = fopen("../../resource/EasyPusher-master/EasyPusher-master/x64/EasyPusher.264", "wb");				
			if (g_pFile == NULL)		
			{			
				printf("CreateFileHead fail\n");			
				return false;
			}	
				//fwrite(pPacketBuffer, sizeof(unsigned char), nPacketSize, g_pFile);
				printf("buffer size = %d \n",nPacketSize);
			
				printf("%s ",pPacketBuffer);
				
		}
	
	else
		{	
			if(g_pFile != NULL)		
			{			
				fwrite(pPacketBuffer+12, sizeof(unsigned char), nPacketSize-12, g_pFile);			
				printf("write data len=%d\n", nPacketSize);		
			}	
		}

}

void CALLBACK g_RealDataCallBack_V30(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,void* dwUser)
{

	printf("sizeof  pBuffer : %ld \n",dwBufSize);
	getvideobuf((unsigned char*)pBuffer,(unsigned int)dwBufSize,(int)dwDataType);
	//writefile(pBuffer,dwBufSize,dwDataType);
	//if(tansform(pBuffer,dwBufSize,dwDataType,timestamp))
	//return;
	
}
void cbStdDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,DWORD dwUser)
{
	writefile(pBuffer,dwBufSize,dwDataType);
	//if(tansform(pBuffer,dwBufSize,dwDataType,timestamp))
	return;
}

//使用UDP方式发送数据
void CALLBACK g_RealDataCallBack_V30_udp(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,void*  dwUser)
{
    printf("pyd---(private_v30)Get data,the size is %d,%d.\n", time(NULL), dwBufSize);
	/*
	if (dwDataType  == NET_DVR_SYSHEAD)
	{	
		printf("dwDataType  == NET_DVR_SYSHEAD \n");
		//写入头数据
		g_pFile = fopen("../record/ps.dat", "wb");
		
		if (g_pFile == NULL)
		{
			printf("CreateFileHead fail\n");
			return;
		}

		//写入头数据
		fwrite(pBuffer, sizeof(unsigned char), dwBufSize, g_pFile);
		printf("write head len=%d\n", dwBufSize);
	}
	else
	{
		if(g_pFile != NULL)
		{
			fwrite(pBuffer, sizeof(unsigned char), dwBufSize, g_pFile);
			printf("write data len=%d\n", dwBufSize);
		}
	}
	*/

	struct sockaddr_in addr;
	int addr_len = sizeof(struct sockaddr_in);   
	bzero(&addr, sizeof(addr));
	addr.sin_family = AF_INET;
	addr.sin_port = htons(UDP_TEST_PORT);
	addr.sin_addr.s_addr = inet_addr(UDP_SERVER_IP);

	
	if( sock_fd  != NULL)
	{
		sendto(sock_fd, pBuffer, dwBufSize, 0, (struct sockaddr *)&addr, addr_len);
	}
	else
		return;
}
void CALLBACK g_HikDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,DWORD dwUser)
{
    printf("pyd---(private)Get data,the size is %d.\n", dwBufSize);
}

void CALLBACK g_StdDataCallBack(int lRealHandle, unsigned int dwDataType, unsigned char *pBuffer, unsigned int dwBufSize, unsigned int dwUser)
{
    printf("pyd---(rtsp)Get data,the size is %d.\n", dwBufSize);
}



/*******************************************************************
      Function:   Demo_GetStream
   Description:   preview(no "_V30")
     Parameter:   (IN)   none  
        Return:   0--successful，-1--fail。   
**********************************************************************/
int Demo_GetStream()
{

    NET_DVR_Init();
    long lUserID;
    //login
    NET_DVR_DEVICEINFO struDeviceInfo;
    lUserID = NET_DVR_Login("192.168.1.2", 8000, "admin", "12345", &struDeviceInfo);
    if (lUserID < 0)
    {
        printf("pyd1---Login error, %d\n", NET_DVR_GetLastError());
        return HPR_ERROR;
    }

    //Set callback function of getting stream.
    long lRealPlayHandle;
    NET_DVR_CLIENTINFO ClientInfo = {0};
#if (defined(_WIN32) || defined(_WIN_WCE))
    ClientInfo.hPlayWnd     = NULL;
#elif defined(__linux__)
    ClientInfo.hPlayWnd     = 0;
#endif

    ClientInfo.lChannel     = 1;  //channel NO
    //ClientInfo.lLinkMode  = 0x40000000; //Record when breaking network.
    ClientInfo.lLinkMode    = 0;
    ClientInfo.sMultiCastIP = NULL;

    lRealPlayHandle = NET_DVR_RealPlay(lUserID, &ClientInfo);
    if (lRealPlayHandle < 0)
    {
        printf("pyd1---NET_DVR_RealPlay_V30 error\n");
        NET_DVR_Logout(lUserID);
        NET_DVR_Cleanup();
        return HPR_ERROR;
    }
    
    //Set callback function of getting stream.
    int iRet;
    iRet = NET_DVR_SetRealDataCallBack(lRealPlayHandle, g_HikDataCallBack, 0);
    if (!iRet)
    {
        printf("pyd1---NET_DVR_RealPlay_V30 error\n");
        NET_DVR_StopRealPlay(lRealPlayHandle);
        NET_DVR_Logout(lUserID);
        NET_DVR_Cleanup();  
        return HPR_ERROR;
    }

	return HPR_OK;
#ifdef _WIN32
    Sleep(5000);  //millisecond
#elif  defined(__linux__)
    sleep(500);   //second
#endif

    //stop
    NET_DVR_StopRealPlay(lRealPlayHandle);
    NET_DVR_Logout(lUserID);
    NET_DVR_Cleanup();
    return HPR_OK;

}

void CALLBACK g_ExceptionCallBack(DWORD dwType, LONG lUserID, LONG lHandle, void *pUser)
{
    char tempbuf[256] = {0};
    switch(dwType) 
    {
    case EXCEPTION_RECONNECT:			//预览时重连
        printf("pyd----------reconnect--------%d\n", time(NULL));
        break;
    default:
        break;
    }
};


/*******************************************************************
      Function:   Demo_GetStream_V30
   Description:   preview(_V30)
     Parameter:   (IN)   none  
        Return:   0--successful，-1--fail。   
**********************************************************************/
int Demo_GetStream_V30(LONG lUserID)
{
	printf("Demo_GetStream_V30 ********************************************************************\n");

    long lRealPlayHandle;
    NET_DVR_CLIENTINFO ClientInfo = {0};
#if (defined(_WIN32) || defined(_WIN_WCE)) || defined(__APPLE__)
    ClientInfo.hPlayWnd     = NULL;
#elif defined(__linux__)
   ClientInfo.hPlayWnd     = 0;
#endif

    ClientInfo.lChannel     = 2;  //channel NO.
    //ClientInfo.lLinkMode    = 0x40000000; //Record when breaking network.
    ClientInfo.lLinkMode    = 0;
    ClientInfo.sMultiCastIP = NULL;

    //lRealPlayHandle = NET_DVR_RealPlay_V30(lUserID, &ClientInfo, g_RealDataCallBack_V30, NULL, 0);
	timestamp = 0;
	
	NET_DVR_PREVIEWINFO ClientInfo1 = {0};
   	ClientInfo1.lChannel     = 0;
	ClientInfo1.dwStreamType = 0;
	ClientInfo1.hPlayWnd = 0;
	ClientInfo1.byPreviewMode =0;
	ClientInfo1.byProtoType = 1;
	lRealPlayHandle = NET_DVR_RealPlay_V40(lUserID, &ClientInfo1, g_RealDataCallBack_V30, NULL);
	
	NET_DVR_SetStandardDataCallBack(lRealPlayHandle, cbStdDataCallBack, 0);
	/*
	if(SocketInit() == -1){ 
		printf("socket init failed !!!!\n");
        return HPR_ERROR;  
    } 
    */
    //lRealPlayHandle = NET_DVR_RealPlay_V30(lUserID, &ClientInfo, NULL, NULL, 0);
    if (lRealPlayHandle < 0)
    {
        printf("pyd1---NET_DVR_RealPlay_V30 error\n");
        return HPR_ERROR;
    }


#ifdef _WIN32
    Sleep(3000);  //millisecond
#elif  defined(__linux__) ||defined(__APPLE__)
    sleep(20);    //second
#endif
	printf("close sock and cleanup !!!!\n");
    //stop
    //NET_DVR_StopRealPlay(lRealPlayHandle);
	//NET_DVR_Logout(lUserID);
	//NET_DVR_Logout_V30(lUserID);
	//cleanup
	close(sock_fd);
	sock_fd = NULL;
	NET_DVR_Cleanup();
   
    return HPR_OK;
}



void Demo_SDK_Ability()
{
    NET_DVR_Init();
    NET_DVR_SDKABL struSDKABL = {0};
    if (NET_DVR_GetSDKAbility(&struSDKABL))
    {
        printf("SDK Max: %d\n", struSDKABL.dwMaxRealPlayNum);
        NET_DVR_Cleanup();
        return;
    }
    NET_DVR_Cleanup();
    return;
}

void Demo_SDK_Version()
{
    unsigned int uiVersion = NET_DVR_GetSDKBuildVersion();

    char strTemp[1024] = {0};
    sprintf(strTemp, "HCNetSDK V%d.%d.%d.%d\n", \
        (0xff000000 & uiVersion)>>24, \
        (0x00ff0000 & uiVersion)>>16, \
        (0x0000ff00 & uiVersion)>>8, \
        (0x000000ff & uiVersion));
    printf(strTemp);
}

